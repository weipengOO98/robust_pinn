"""
while true;do python unsteady_NS_two_stage.py --adam_iter=10000  --save_path='./data/test_two_stage';done
"""
...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, t, u, v, layers, ExistModel=0, uvDir='', loss_type='square', bfgs_iter=-1):
        self.count = 0
        ...
        if ExistModel == 0:
            ...
            print("Loading uv NN ...")
            self.uv_weights, self.uv_biases = self.load_NN(uvDir, self.layers)
        ...
        self.data_error = tf.sqrt(tf.square(self.u_tf - self.u_pred) + tf.square(self.v_tf - self.v_pred))
        self.loss_DATA_sq = tf.reduce_mean(tf.square(self.u_tf - self.u_pred)) \
                            + tf.reduce_mean(tf.square(self.v_tf - self.v_pred))
        self.loss_DATA_l1 = tf.reduce_mean(tf.abs(self.u_tf - self.u_pred)) \
                            + tf.reduce_mean(tf.abs(self.v_tf - self.v_pred))
        self.loss = self.weight_tf * self.loss_f + (
                    1 - self.switch_tf) * self.loss_DATA_l1 + self.switch_tf * self.loss_DATA_sq
    ...
    def train(self, adam_iter, learning_rate, weight, switch):
        ...
        for it in range(adam_iter):
            loss_total, loss_data_sq, loss_pde, lambda_1, lambda_2, _ = self.sess.run(
                [self.loss, self.loss_DATA_sq, self.loss_f, self.lambda_1, self.lambda_2, self.train_op_Adam], tf_dict)
            self.callback(loss_total, loss_data_sq, loss_pde, lambda_1, lambda_2)
    ...
    def train_bfgs(self, weight, switch):
        ...
        self.optimizer.minimize(self.sess,
                                feed_dict=tf_dict,
                                fetches=[self.loss, self.loss_DATA_sq, self.loss_f, self.lambda_1, self.lambda_2],
    ...
    def sieve_obs(self, ratio):
        ...
        self.v = self.v[ind, :]
        error = self.sess.run(self.data_error, tf_dict).ravel()
        print("sieve:", self.x.shape, self.y.shape, self.t.shape, self.u.shape, self.v.shape, )
        return error
    ...
    def plot_result(self, filename):
        ...
        im = ax[3].pcolormesh(x.ravel().reshape(*_shape), y.ravel().reshape(*_shape),
                              (p_pred - p_mean_slice).ravel().reshape(*_shape), )
        ...
        im = ax[5].pcolormesh(x.ravel().reshape(*_shape), y.ravel().reshape(*_shape),
                              (p_pred - p_mean_slice).ravel().reshape(*_shape) - p.ravel().reshape(*_shape), cmap='bwr')
...
def run_experiment(epoch_num, noise_type, noise, loss_type, weight, N=5000, _data=[], l_size=-1, abnormal_size=0,
    ...
    with tf.device('/device:GPU:0'):
        ...
        if (sieve_sigma == 0) and (sieve_ratio == 0):
            model = PhysicsInformedNN(x_train, y_train, t_train, u_train, v_train, layers=uv_layers,
                                      ExistModel=0,
                                      uvDir='uvNN.pickle',
                                      loss_type=loss_type,
                                      bfgs_iter=1000)
            ...
            print(str(path) + '/' + f"*_l1_{N}_{noise_type}_{noise}_{abnormal_size}_*_0_0.npz")
            model_ref = PhysicsInformedNN(x_train, y_train, t_train, u_train, v_train, layers=uv_layers,
                                          ExistModel=1,
                                          uvDir=glob.glob(str(
            ...
            model = PhysicsInformedNN(x_train, y_train, t_train, u_train, v_train, layers=uv_layers,
                                      ...
                                      bfgs_iter=1000)
            if sieve_sigma > 0 and sieve_ratio == 0:
                # model_ref.sieve_obs_sigma(k=sieve_sigma)
                model.sieve_obs_sigma(k=sieve_sigma)
            elif sieve_sigma == 0 and sieve_ratio > 0:
                # model_ref.sieve_obs(ratio=sieve_ratio)
        ...
        with open(path.joinpath('result.csv'), 'a+') as f:
            f.write(
                f"{epoch_num},{loss_type},{N},{noise_type},{noise},{abnormal_size},{weight},{error_u},{error_v},{error_vel},{error_p},{lambda_1_error}, {lambda_2_error},{sieve_sigma},{sieve_ratio},{len(model.x)}\n")
