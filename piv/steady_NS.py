...
# S_alpha: float = float(args.S_alpha)
...
class PINN_laminar_flow_robust:
    ...
    def __init__(self, Collo, OUTLET, WALL, DATA, uv_layers, lb, ub, ExistModel=0, uvDir='', loss_type='square'):
        ...
        if loss_type == 'square':
            self.loss_DATA = tf.reduce_mean(tf.square(self.u_pred_train - self.u_train)) \
                             + tf.reduce_mean(tf.square(self.v_pred_train - self.v_train))
        elif loss_type == 'l1':
            self.loss_DATA = tf.reduce_mean(tf.abs(self.u_pred_train - self.u_train)) \
                             + tf.reduce_mean(tf.abs(self.v_pred_train - self.v_train))
        else:
            raise NotImplementedError(f'Loss type {loss_type} not implemented.')
        self.loss = self.weight_tf * self.loss_f + self.loss_WALL + self.loss_OUTLET + self.loss_DATA
...
def run_experiment(epoch_num, noise_type, noise, loss_type, N, weight, _data=[], l_size=-1, abnormal_size=0):
    ...
    with tf.device('/device:GPU:0'):
        ...
        with open(path.joinpath('result.csv'), 'a+') as f:
            f.write(
                f"{epoch_num},{loss_type},{N},{noise_type},{noise},{abnormal_size},{weight},{error_u},{error_v},{error_vel}, {error_p}\n")
